...
public class ReservaService {
    ...
    public ArrayList<Reserva> listarReservas() {
        ...
        for(int i = 0; i < lineas.size(); i++){
            ...
            reserva.add(new Reserva(Integer.parseInt(arrLinea[0]), Integer.parseInt(arrLinea[1]), Integer.parseInt(arrLinea[2]), Integer.parseInt(arrLinea[3]),(arrLinea[4]), (arrLinea[5])));
        }
        
        return reserva;
    }
    
    public void createReserva(int fk_Usuario, int fk_Funcion, int fk_Asiento, String estado, String fecha_Reserva ) {
        editor.crearArchivo(FILENAME);
        int id = editor.getUltimoId(FILENAME, this.SEPARADOR) + 1;
        String registro = String.format("%d;%d;%d;%d;%s;%s", id, fk_Usuario, fk_Funcion, fk_Asiento, estado, fecha_Reserva);
    ...
    public Reserva getReservaById(int reserva_id){
        ...
        return new Reserva(Integer.parseInt(fLinea[0]), Integer.parseInt(fLinea[1]), Integer.parseInt(fLinea[2]), Integer.parseInt(fLinea[3]),(fLinea[4]), (fLinea[5]));
    }
    
    public Reserva updateReserva(int idReserva, int fk_Usuario, int fk_Funcion, int fk_Asiento, String estado, String fecha_Reserva){
        ...
        String reserva_updated =String.format("%d;%d;%d;%d;%s;%s", reserva.getIdReserva(), fk_Usuario, fk_Funcion, fk_Asiento, estado, fecha_Reserva);
